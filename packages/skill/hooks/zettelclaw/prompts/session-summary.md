...
- **Session footer format** — ensure there is a `---` divider followed by `## Session Sources`, then append `- SESSION_ID — HH:MM`.
- **Idempotency** — if `SESSION_ID` already appears in `## Session Sources`, do NOT add duplicate bullets or sources. Return a message saying the session was already captured.
